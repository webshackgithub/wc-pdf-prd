...
- [ ] `npm run dev` 구동 시 에러 없이 메인 화면이 표시되는가?
- [ ] 임시 버튼을 만들어 클릭 시 `Upload` -> `Processing` -> `Success` 뷰가 정상적으로 전환되는가?
- [ ] 반응형(모바일/데스크탑)에서 레이아웃이 깨지지 않는가?
...
- [ ] 이미지 파일(.jpg)을 드래그했을 때 거부되고 경고 메시지가 뜨는가?
- [ ] 100MB 이상의 대용량 파일을 올렸을 때 제한되는가?
- [ ] 정상적인 PDF 파일을 올렸을 때 `Processing` 화면으로 자동 전환되는가?
...
- [ ] (단위 테스트 또는 콘솔 로그) 테스트 PDF 파일을 함수에 넣었을 때, 페이지 수만큼의 배열이 반환되는가?
- [ ] 생성된 ZIP Blob의 크기가 0이 아니고 유효한가?
- [ ] 50페이지 이상의 중형 PDF 처리 시 브라우저가 멈추지 않는가(비동기 처리 확인)?
...
### 4.1 로직과 UI 통합
- [ ] `ProcessingView`에서 실제 분할 작업 실행
- [ ] **진행률 표시 (Optional):** 현재 처리 중인 페이지 수 등을 UI에 업데이트 (가능하다면)
- [ ] 처리 완료 시 결과 ZIP Blob을 상태에 저장하고 `COMPLETED` 상태로 전환

### 4.2 다운로드 기능 (`SuccessView`)
- [ ] **라이브러리 설치:** `npm install file-saver` (및 `@types/file-saver`)
- [ ] **다운로드 버튼:** 클릭 시 `saveAs(zipBlob, "filename.zip")` 실행
- [ ] **결과 요약:** "총 N개의 페이지로 분할되었습니다" 문구 표시

### 4.3 리셋(Reset) 기능
- [ ] "처음으로" 또는 "다른 파일 변환하기" 버튼 구현
- [ ] 상태 초기화 (`file = null`, `status = IDLE`, `zipBlob = null`)

### 4.4 최종 UI 폴리싱
- [ ] Transition 효과: 상태 전환 시 부드러운 애니메이션
- [ ] 모바일 UI 확인: 작은 화면에서 버튼이나 카드가 잘리지 않는지 확인
- [ ] 메타데이터: `metadata` API를 사용해 Title, Description 설정

### ✅ Phase 4 검증 (Verification)
- [ ] [E2E] 업로드 -> 분할(로딩) -> 완료 -> 다운로드 -> 리셋 -> 재업로드 과정이 매끄러운가?
- [ ] 다운로드된 ZIP 파일이 정상적으로 압축 해제되고 PDF들이 열리는가?
- [ ] 한글 파일명 처리가 깨지지 않고 잘 되는가?
